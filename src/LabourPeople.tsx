...
import { createLabourPerson, getLabourPeople } from './api'
...
// Names and hours only — no rate, no Budget treatment.
        ...
        {shown.map(p => (
          <li key={p.id}>
            <div className="labour-person-row labour-person-row--static">
              ...
              {p.jobHoursLabel && <span className="labour-person-figures">{p.jobHoursLabel}</span>}
            </div>
          </li>
        ))}
      </ul>
...
export function ManagePeopleDrawer({ jobId, people, open, onClose, onChanged }: {
  ...
  const addPerson = async () => {
    ...
    try {
      await createLabourPerson(jobId, { name: trimmed, defaultBudgetTreatment: 'hours_only' })
  ...
  return (
    <BottomSheet title="People" onClose={onClose}>
      <div className="manage-people">
        <p className="row-sheet-sub">Who has worked on this job. Used to attribute hours.</p>
