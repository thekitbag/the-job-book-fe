import type { AlreadyRememberedItem, AuthUser, BudgetCategory, BudgetSummaryResponse, CandidateFact, CreateBudgetCategoryRequest, CreateMemoryItemRequest, ExtractionStatus, InspectionData, Job, JobType, LocalNote, MemoryItemEdit, MemoryType, MemoryViewItem, MemoryViewResponse, MemoryViewSection, PatchBudgetCategoryRequest, QueueDecision, QueueDecisionResponse, QueueItem, ReviewQueue, TranscriptStatus } from './types'
...
export async function signup(email: string, password: string, name?: string): Promise<AuthUser> {
  ...
  if (USE_MOCK) {
    await delay(300)
    if (mockAccounts.has(normalized)) throw new ApiError(AUTH_ERROR_COPY.EMAIL_IN_USE, 409)
    const user: AuthUser = { id: `user-mock-${Date.now()}`, email: normalized, name: name?.trim() || 'Builder', role: 'PILOT' }
