 ...
 */
export default function AddSheet({ title, onClose, onRecordInstead, children }: {
  title: string
  onClose: () => void
  ...
  return createPortal(
    ...
    <div className="add-sheet-backdrop" onClick={onClose}>
      ...
      <div className="add-sheet" role="dialog" aria-modal="true" aria-label={title} onClick={e => e.stopPropagation()}>
        ...
        <div className="add-sheet-body">{children}</div>
        {onRecordInstead && (
          <p className="add-sheet-voice">
            <span aria-hidden="true">🎙</span> Prefer to say it?{' '}
            <button type="button" className="add-sheet-voice-link" onClick={onRecordInstead}>Record instead</button>
          </p>
        )}
      </div>
    </div>,
