...
test.describe('Supplier account settlement', () => {
  test('tick, mark paid, and read what the payment covered', async ({ page }) => {
    ...
    await expect(bar(page)).toContainText('£3,860 on the account')
    ...
    await expect(bar(page)).toContainText('£550 left unpaid')
    await expectNoHorizontalOverflow(page)
    ...
    await expect(page.getByRole('heading', { name: /£550/ })).toBeVisible()
  ...
  test('Select all offers the whole account, and Clear takes it back', async ({ page }) => {
    ...
    await expect(bar(page).getByRole('button', { name: 'Mark £3,860 paid' })).toBeEnabled()
